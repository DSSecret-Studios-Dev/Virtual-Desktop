...
CARD_SPACING_X = 110
CARD_BACK = QImage(os.path.join('./Solitaire/images', 'back.jpg'))
...
class Card(QGraphicsPixmapItem):
    ...
    def load_images(self):
        self.face = QPixmap(
            os.path.join('./Solitaire/cards', '%s%s.png' % (self.value, self.suit))
        )

        self.back = QPixmap(
            os.path.join('./Solitaire/images', 'back.jpg')
        )
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        felt = QBrush(QPixmap(os.path.join('./Solitaire/images', 'felt.png')))
